...
export const favoritesProgramSlice = createSlice({
  ...
  reducers: {
    addItem(state, action) {
      ...
      const existingItem = state.cart.find(item => item.id && item.title == newItem.id && newItem.title)
    ...
    removeItem(state, action) {
      ...
      state.cart = state.cart.filter(item => item.id && item.title !== removItem.id && removItem.title)
    }
  }
})
